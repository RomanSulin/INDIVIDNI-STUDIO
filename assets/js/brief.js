(() => {
  ...
  const STORAGE_KEY = "individni_brief_v1";
  ...
  const steps = [
    ...
    { key:"channels", title:"Где будет использоваться", kicker:"Шаг 2", hint:"Можно выбрать несколько.", color:COLORS[2], kind:"multi", options:["YouTube / Web","Соцсети","TV","Сцена / экран","Внутренние презентации"], required:false },
    ...
    { key:"refs", title:"Материалы и референсы", kicker:"Шаг 7", hint:"Ссылки на папку/рефы/доки. Можно позже.", color:COLORS[8], kind:"text", label:"Ссылки (Drive/Dropbox/YouTube/Notion)", required:false },
    { key:"done", title:"Готово", kicker:"Шаг 8", hint:"Проверьте ответы и отправьте.", color:COLORS[0], kind:"summary", required:false }
  ];
  ...
  function renderProgress(){
    ...
    steps.forEach((s, i) => {
      ...
      btn.type = "button";
      btn.className = "prog-dot" + (i === state.idx ? " is-active" : "") + (i < state.maxIdx ? " is-done" : "");
      btn.disabled = !canGo(i);
      btn.innerHTML = `<span class="prog-dot__n">${i+1}</span><span>${s.title}</span>`;
  ...
  function renderSide(){
    ...
    steps.forEach((s, i) => {
      ...
      const isDone = i < state.maxIdx;
      item.className = "step-chip" + (isActive ? " is-active" : "") + (isDone ? " is-done" : "");
      item.innerHTML = `
        <div class="step-chip__n">${i+1}</div>
        <div class="step-chip__t">${s.title}</div>
        <div class="step-chip__s">${isDone ? "готово" : (isActive ? "сейчас" : "")}</div>
      `;
      item.addEventListener("click", () => setIdx(i));
      sideSteps.appendChild(item);
    });
  }
