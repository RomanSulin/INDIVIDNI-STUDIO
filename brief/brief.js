/* INDIVIDNI Brief — dashboard style
   - Tabs + sidebar steps
   - LocalStorage
*/
(() => {
  ...
  const STORAGE_KEY = "individni_brief_dash_v1";
  ...
  const steps = [
    ...
    { key:"done", title:"Готово", kicker:"Шаг 8", hint:"Проверьте ответы и экспортируйте.", color:COLORS[0], kind:"summary", required:false }
  ];
  ...
  function renderTabs(){
    ...
    steps.forEach((s, i) => {
      ...
      btn.type = "button";
      btn.className = "tab" + (i === state.idx ? " is-active" : "") + (i < state.maxIdx ? " is-done" : "");
      btn.disabled = !canGo(i);
      btn.innerHTML = `<span class="tab__n">${i+1}</span><span>${s.title}</span>`;
  ...
  function renderSide(){
    ...
    steps.forEach((s, i) => {
      ...
      item.className = "step" + (isActive ? " is-active" : "") + (isDone ? " is-done" : "");
      item.innerHTML = `
        <div class="step__n">${i+1}</div>
        <div class="step__t">${s.title}</div>
        <div class="step__s">${isDone ? "готово" : (isActive ? "сейчас" : "")}</div>
      `;
  ...
  function updateMini(){
    ...
    const done = Math.max(0, Math.min(steps.length, state.maxIdx));
    doneCountEl.textContent = String(done);
    totalCountEl.textContent = String(steps.length);
  ...
  function render(dir){
    ...
    setTimeout(() => {
      card.className = "stage__card " + inClass;
      card.innerHTML = "";
